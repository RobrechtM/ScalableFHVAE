...
class RegFHVAE(object):
    def __init__(self, xin, xout, y, n, nmu2, bReg, cReg, b_nlabs, c_nlabs):
        ...
        self.mu2_table, self.mu2, self.qz2_x, self.z2_sample, self.qz1_x, \
                self.z1_sample, self.px_z, self.x_sample, self.z1_rlogits, self.z2_rlogits = \
                self.net(
                    self.xin, self.xout, self.y, self.nmu2, self.z1_rhus, 
                    self.z1_dim, self.z2_rhus, self.z2_dim, self.x_rhus, b_nlabs, c_nlabs) #self.cReg.get_shape().as_list()[-1])
        ...
        TensorList=[tf.expand_dims(-fn(labels=tf.squeeze(tf.slice(self.bReg,[0,i],[-1,1]),axis=-1),\
                                       logits=self.z1_rlogits[i]),axis=1) for i, name in enumerate(b_nlabs.keys())]
        self.log_b = tf.concat(TensorList, axis=1)
        TensorList=[tf.expand_dims(-fn(labels=tf.squeeze(tf.slice(self.cReg,[0,i],[-1,1]),axis=-1),\
                                       logits=self.z2_rlogits[i]),axis=1) for i, name in enumerate(c_nlabs.keys())]
    ...
    def net(self, xin, xout, y, nmu2, z1_rhus, z1_dim, z2_rhus, z2_dim, x_rhus, z1_nlabs, z2_nlabs):
        ...
        with tf.variable_scope("fhvae"):
            ...
            #z1_rlogits = [reg_predict(z1_sample, lab) for lab in z1_nlabs.items()]
            #z2_rlogits = [reg_predict(z2_sample, lab) for lab in z2_nlabs.items()]
            z1_rlogits = [reg_predict(z1_mu, lab) for lab in z1_nlabs.items()]
            z2_rlogits = [reg_predict(z2_mu, lab) for lab in z2_nlabs.items()]
            # rlogits are indexed with 0, 1, 2, ...

        return mu2_table, mu2, qz2_x, z2_sample, qz1_x, z1_sample, px_z, x_sample, z1_rlogits, z2_rlogits
